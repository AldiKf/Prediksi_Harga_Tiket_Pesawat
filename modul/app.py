...
page = st.sidebar.radio(
    "Pilih Halaman",
    ["Prediksi Harga", "Analisis Data"],
    label_visibility="collapsed"
)

if page == "Prediksi Harga":
    st.sidebar.success("Anda berada di halaman Prediksi Harga.")
# CSS (LIGHT MODE)
    st.markdown("""
    <style>
...
# ============================================================ Analisis Page ===========================================================
    ...
    st.sidebar.success("Anda berada di halaman Analisis Data Maskapai.")
    st.markdown("""
    <div style="display:flex; justify-content:space-between; align-items:center;">
    <div>
    ...
    if uploaded_file:
        ...
        # Deteksi Kolom yang ada pada csv
        numeric_cols = df.select_dtypes(include=["int64", "float64"]).columns.tolist()
        categorical_cols = df.select_dtypes(include=["object"]).columns.tolist()

        st.divider()

        # Pilih kolom untuk dianalissis
        ...
        def detect_unit(col_name):
            ...
            if "harga" in col or "price" in col:
                return "rupiah"
            elif "km" in col or "jarak" in col or "distance" in col:
                return "km"
            else:
                return "number"
        ...
        unit_type = detect_unit(selected_numeric)

        total_data = len(df)
        mean_val = df[selected_numeric].mean()
        min_val = df[selected_numeric].min()
        max_val = df[selected_numeric].max()
        ...
        with col2:
            ...
            <div style="padding:16px;border-radius:12px;background:#f8f9fa">
                ...
                <h3>{format_value(mean_val, unit_type)}</h3>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col3:
            ...
            <div style="padding:16px;border-radius:12px;background:#f8f9fa">
                <small>Minimum</small>
                <h3>{format_value(min_val, unit_type)}</h3>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col4:
            ...
            <div style="padding:16px;border-radius:12px;background:#f8f9fa">
                ...
                <h3>{format_value(max_val, unit_type)}</h3>
            </div>
            """, unsafe_allow_html=True)
        ...
        with st.container():
            ...
            with col1:
                fig, ax = plt.subplots()
                ax.hist(df[selected_numeric], bins=30)
            ...
            # Analisis PerKategori
            if selected_category != "None":
                st.divider()
                st.subheader("🏷️ Analisis Berdasarkan Kategori")
